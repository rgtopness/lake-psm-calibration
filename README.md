[![DOI](https://zenodo.org/badge/DOI/10.5281/zenodo.19898086.svg)](https://doi.org/10.5281/zenodo.19898086)
...
The lake model is based on Hostetler & Bartlein (1990), with adaptions from Morrill et al. (2019) and Topness et al. (in prep). For a complete description of the lake model and runoff-isotope module, see Topness et al. (in prep).
...
### (1) Edit the model set-up files located in the `model/01_set_up_files` folder
...
* Meteorological input file: Model forcing, either from observations, reanalysis data, or climate model output. Must be a .txt file but file name does not matter. See `model/01_set_up_files` for an example meterological input file on a daily timestep. File columns should be in the following order:
