...
!-----------------------------------------------------------------------
      ...
      if (wb_flag) then
         ...
         call water_balance (nzlk, dfrac, rain, evapw * (1. - fracprv), &
              ...
              n_slice, isave_d, tracer, run_len, runout_len,fracprv)
...
!==============================================================
!   12  DO d18O AND d2H BALANCE FOR OPEN WATER ONLY
!==============================================================

      if (d18O_flag) then
        if (fracprv.lt.1) then
           call D18O (run_len, runout_len, rain, evapw * (1. - fracprv),  &
                rh, tlake, tracer, d18Op, d18Or, d18Os, dsnow)
        else
           call D18O (run_len*0.0, runout_len, rain*0.0, &
               evapw * (1. - fracprv), rh, tlake, tracer, d18Op, d18Or, &
               d18Os, dsnow * 0.0)
        endif
      endif
      if (d2H_flag) then
        if (fracprv.lt.1) then
           ...
           call D2H (run_len*0.0, runout_len, rain*0.0, evapw* (1. - fracprv),&
                rh, tlake, tracer, d2Hp, d2Hr, d2Hs, dsnow*0.0)
        endif
      endif
